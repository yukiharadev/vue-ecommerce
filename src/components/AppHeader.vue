<template>
  <div class="bg-black text-white">
    <div class="container text-center mx-auto py-1 text-xs">
      <slot>Sign up and get 20% off to your first order. </slot>
      <RouterLink to="/auth/signup" class="underline font-bold cursor-pointer">Sign Up Now</RouterLink>
    </div>
  </div>
  <div class="bg-white">
    <fwb-navbar class="container max-w-7xl mx-auto">
      <template #logo>
        <RouterLink to="/" class="font-bold font-mono text-xl">YukiharaShop</RouterLink>
      </template>
      <template #default="{ isShowMenu }">
        <fwb-navbar-collapse :is-show-menu="isShowMenu">
          <RouterLink class="fwb-navbar-link" is-active="true" to="/">
            Shop
          </RouterLink>
          <RouterLink class="fwb-navbar-link" to="/on-sale">
            On Sale
          </RouterLink>
          <RouterLink class="fwb-navbar-link" to="/new-arrivals">
            New Arrivals
          </RouterLink>
          <RouterLink class="fwb-navbar-link" to="/brands"> Brand </RouterLink>
        </fwb-navbar-collapse>
      </template>
      <template #right-side>
        <fwb-input placeholder="Search for Products....." size="md"
          class="!rounded-full bg-[#F2F0F1] hidden lg:inline-flex w-full lg:w-96 xl:w-[39rem] transition-all duration-300 ease-in-out">
          <template #prefix>
        ...
        <RouterLink to="auth/signin" class="ml-3">
